...
namespace RageLib.Models.Resource
{
    ...
    public class FragTypeModel : IFileAccess, IDataReader, IEmbeddedResourceReader, IDisposable
    {
        public class FragTypeChild : IFileAccess
        {
            ...
            /// <summary>
            /// Index into FragTypeModel.Skeleton.Bones identifying which bone this piece
            /// is attached to — that bone's Position/RotationQuaternion give the piece's
            /// transform relative to the fragment origin.
            ///
            /// Field location determined from GTAIV.exe Ghidra decomp: the rage::fragTypeChild
            /// constructor (FUN_00631b00 in the decompilation) accepts the bone tag as a
            /// ushort parameter and writes it at byte offset 0x0e of the struct. Callers
            /// (e.g. FUN_0063... at line 251271) then re-set the same offset with their own
            /// bone-index value, confirming it as a persistent, caller-specified field.
            /// </summary>
            public short BoneIndex { get; private set; }

            /// <summary>Flags byte at offset 0x0c. Same callers set this independently.</summary>
            ...
            public void Read(BinaryReader br)
            {
                ...
                // Struct layout (128 bytes of header before the Drawable pointer at 0x90):
                //   0x00 : vtable/ptr
                //   0x04 : unknown 8 bytes (possibly bounding sphere center + radius)
                //   0x0c : Flags (1 byte)
                //   0x0d : padding (1 byte)
                //   0x0e : BoneIndex (short)
                //   0x10 - 0x8f : other per-child data (physics params, possibly a local matrix)
                //   0x90 : Drawable offset (uint)
                br.BaseStream.Seek(startPos + 0x0c, SeekOrigin.Begin);
                Flags = br.ReadByte();
                br.BaseStream.Seek(startPos + 0x0e, SeekOrigin.Begin);
                BoneIndex = br.ReadInt16();

                br.BaseStream.Seek(startPos + 0x90, SeekOrigin.Begin);
