    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // 2-layer splat: green channel paints layer 1 over the base. Same channel
                // mapping the 3lyr shader uses for its first overlay so the artist's paint
                // looks consistent across variants.
                half wOverlay = saturate(IN.color.g);
                half wBase    = 1.0 - wOverlay;

                half4 albedo = c0 * wBase + c1 * wOverlay;
